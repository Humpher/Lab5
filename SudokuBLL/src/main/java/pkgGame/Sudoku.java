...
public class Sudoku extends LatinSquare implements Serializable {
	...
	private HashMap<Integer, SudokuCell> theRemainingCells = new HashMap<Integer, SudokuCell>();
	...
	private int PossibleValuesMultiplier(HashMap<Integer, SudokuCell> theRemainingCells) {
		
		int possValMulti = 1;
		for(int iRow = 0; iRow < iSize; iRow++) {
			for(int iCol = 0; iCol< iSize; iCol++) {
				SudokuCell c = theRemainingCells.get(Objects.hash(iRow,iCol));
			if(c.getLstValidValues().size() != 0) {
				possValMulti = possValMulti * c.getLstValidValues().size();
			}
		}
		}
		return possValMulti;
	...
	private void SetRemainingCells() {
		for(int iRow = 0; iRow < iSize; iRow++) {
			for(int iCol = 0; iCol< iSize; iCol++) {
				...
				theRemainingCells.put(c.hashCode(),c);
			}
		}
	}
	...
	public void RemoveCells() {
		SetRemainingCells();
		while(isDifficultyMet(PossibleValuesMultiplier(theRemainingCells))) {
	...
	private boolean isDifficultyMet(int difficulty) {
		...
		if(eGameDifficulty.getiDifficulty()<=difficulty)
			return true;
		else
			return false;
	}
